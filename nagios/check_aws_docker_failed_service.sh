...
#
# Test this with
# check_aws_docker_failed_service.sh -H  ENG1-cntrain-AppDockerTemlate-XLFGH4XPITAI-rAutoScalingGroupDockerWorker-17KGP77HMTHHM 
...
do
	...
	# we  do that because the "docker servuce ls" command we run will always have a success status even if the service isn't running.
	# the extra grep is an easy way to see if the service really was listed in the output we got back from nrpe.

        respstr=`/usr/local/nagios/libexec/check_nrpe -c check_docker_service_replicas | sed '0/'`
	...
	# we only need to do this for one docker master, so we break out of the loop after the first pass
	break
done
